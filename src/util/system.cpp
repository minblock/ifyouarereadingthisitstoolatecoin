...
const char * const BITCOIN_CONF_FILENAME = "ifyouarereadingthisitstoolatecoin.conf";
...
#else
    const char* pszModule = "ifyouarereadingthisitstoolatecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ifyouarereadingthisitstoolatecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ifyouarereadingthisitstoolatecoin";
#else
    // Unix
    return pathRet / ".ifyouarereadingthisitstoolatecoin";
#endif
#endif
}
