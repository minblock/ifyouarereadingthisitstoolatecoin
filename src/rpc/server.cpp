...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"stop",
                "\nStop Ifyouarereadingthisitstoolatecoin server.",
                {},
                RPCResults{},
                RPCExamples{""},
            }.ToString());
    ...
    return "Ifyouarereadingthisitstoolatecoin server stopping";
}
...
{
    return "> ifyouarereadingthisitstoolatecoin-cli " + methodname + " " + args + "\n";
}
