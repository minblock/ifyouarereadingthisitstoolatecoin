...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ifyouarereadingthisitstoolatecoin address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("ifyouarereadingthisitstoolatecoin"))
        return false;
...
{
    QString ret = QString("ifyouarereadingthisitstoolatecoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ifyouarereadingthisitstoolatecoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ifyouarereadingthisitstoolatecoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ifyouarereadingthisitstoolatecoin (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ifyouarereadingthisitstoolatecoin.desktop";
    return GetAutostartDir() / strprintf("ifyouarereadingthisitstoolatecoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ifyouarereadingthisitstoolatecoin\n";
        else
            optionFile << strprintf("Name=Ifyouarereadingthisitstoolatecoin (%s)\n", chain);
